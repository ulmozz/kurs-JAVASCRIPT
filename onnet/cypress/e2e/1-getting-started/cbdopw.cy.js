...
describe('Logowanie do CBDOPW', () => {
  beforeEach(() => {
    cy.visit('http://10.0.2.36/cbdopw')
  })

  it('Wpisuje login i klika Zaloguj', () => {
    // Wpisanie nazwy usera
    cy.get('[data-selenium="login-username"] input')
    .type('CZS1');

    // Znalezienie przycisku Zaloguj i sprawdzenie czy ma tekst "Zaloguj"
    cy.get('[data-selenium="form-btn-login-submit"]')
    .should('contain.text', 'Zaloguj')
    .click();

    // Czekamy aż zniknie "Trwa ładowanie"
    cy.get('.dx-loadpanel-message').should('not.be.visible');

    // Sprawdza czy logowanie się powiodło
    cy.get('[data-selenium="logout"]')
    .should('contain.text', 'CZS1');
  })
});
...
//     const newItem = 'Feed the tatatata'
